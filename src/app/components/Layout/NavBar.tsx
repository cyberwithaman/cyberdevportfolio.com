  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled 
          ? 'bg-gradient-to-br from-[#0A0A0A]/90 via-[#111111]/90 to-[#0f0f23]/90 backdrop-blur-md border-b border-zinc-700/50 shadow-xl shadow-blue-500/5' 
          : 'bg-transparent'
      }`}
    >
      ...
      <div className="nextjs-container relative">
              ...
              <Link
                ...
                className={`px-4 py-2 text-sm font-medium rounded-xl transition-all duration-300 backdrop-blur-sm ${
                  activePath === item.href
                    ? 'text-zinc-100 bg-gradient-to-r from-zinc-900/90 to-zinc-800/90 border border-zinc-700/50 shadow-lg'
                    : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50 hover:border-zinc-700/30 border border-transparent'
                }`}
        ...
        <div
          id="mobile-menu"
          className={`fixed top-0 right-0 bottom-0 w-full max-w-[100%] bg-gradient-to-br from-[#0A0A0A] via-[#111111] to-[#0f0f23] border-l border-zinc-700/50 md:hidden transform transition-transform duration-300 ease-in-out z-40 backdrop-blur-md ${
              ...
              <ul className="space-y-2 max-w-[280px] mx-auto">
                {navItems.map((item) => (
                  <li key={item.name}>                    <Link
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`flex items-center justify-center px-4 py-3.5 text-sm font-medium rounded-xl transition-all duration-300 backdrop-blur-sm ${
                        activePath === item.href
                          ? 'text-zinc-100 bg-gradient-to-r from-zinc-900/90 to-zinc-800/90 border border-zinc-700/50 shadow-lg'
                          : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50 hover:border-zinc-700/30 border border-transparent'
                      }`}
